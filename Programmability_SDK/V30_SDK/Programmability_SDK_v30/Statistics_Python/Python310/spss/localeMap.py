...
def readKeysLocale(spss_locale):
    return [x for x in list(localeDict.keys()) if x.startswith(spss_locale)]

def readValuesLocale(spss_locale):
    return [x for (x, v) in list(localeDict.items()) if [i for i in v if i.startswith(spss_locale)]]
...
def SetSPSSLocaleAsPythonLocale(spss_locale):
    ...
    if py_locale is None:
        ...
        py_locale_specifies = [x for x in list(py_locale_dict.keys()) if x.startswith(CheckStr(spssLocaleSimple))]
