...
class BaseCursor(object):
    ...
    def __del__(self):
        if self.running:
            ...
            for ds in list(PythonDatasets.values()):
    ...
    def GetVarMissingValues(self,index):
       ...
       """
        ...
        vartype = self.GetVariableType(index)
...
class ReadCursor(BaseCursor):
    ...
    def reset(self):
        ...
        error.SetErrorCode(PyInvokeSpss.ResetDataPass())
        if error.IsError():
            raise SpssError(error)
    ...
    def SetFetchVarList(self, var = None):
        ...
        if self.isBinary:
            ...
            error.SetErrorCode(PyInvokeSpss.SetFetchVarList(self.datasrcInfo,self.varTuple))
            if error.IsError():
                raise SpssError(error)
    ...
    def _getDateIndexes(self):
        self.cvtDateIndexes = list(filter(self._isDateVariable, self.cvtDateIndexes))
...
class BaseWriteAppendCursor(object):
    ...
    def SetValueChar(self,varName,varValue):
        ...
        if self.isBinary:
            ...
            error.SetErrorCode(PyInvokeSpss.SetValueChar(varName,varValue))
            if error.IsError():
                raise SpssError(error)
    ...
    def SetValueNumeric(self,varName,varValue):
        ...
        if self.isBinary:
            ...
            error.SetErrorCode(PyInvokeSpss.SetValueNumeric(varName,varValue))
            if error.IsError():
                raise SpssError(error)
...
class WriteCursor(ReadCursor,BaseWriteAppendCursor):
    ...
    def reset(self):
        ...
        error.SetErrorCode(PyInvokeSpss.ResetDataPassWrite(self.conv,self.datasrcInfo,tuple(self.var)))
        if error.IsError():
            raise SpssError(error)
    ...
    def AllocNewVarsBuffer(self,bufSize):
        """Allocate bufSize bytes memory to add new variables.
          ...
          --examples:
            ...
            caseCount = cur.GetCaseCount()
            cur.SetOneVarNameAndType("t1",20)
            cur.CommitDictionary()
            for j in range(caseCount):
                cur.fetchone()
                cur.SetValueChar("t1", "A"*10)
                cur.CommitCase()
        ...
        if bufSize <= 0:
            error.SetErrorCode(1056)
            raise SpssError(error)

        error.SetErrorCode(PyInvokeSpss.AllocNewVarsBuffer(bufSize))
        if error.IsError():
            raise SpssError(error)
    ...
    def SetOneVarNameAndType(self,varName,varType):
        ...
        if not isinstance(varType,int):
            error.SetErrorCode(1000)
            raise SpssError(error)
        if varType < 0:
            error.SetErrorCode(1067)
            raise SpssError(error)

        error.SetErrorCode(PyInvokeSpss.SetOneVarNameAndType(varName,varType))
        if error.IsError():
            raise SpssError(error)
    ...
    def SetVarNameAndType(self,varName,varType):
        ...
        for i in range(len(varType)):
            if not isinstance(varType[i],int):
                error.SetErrorCode(1000)
                raise SpssError(error)
            if varType[i] < 0:
                error.SetErrorCode(1067)
        ...
        if len(varName) != len(varType):
            error.SetErrorCode(1055)
            raise SpssError(error)

        error.SetErrorCode(PyInvokeSpss.SetVarName(varName,varType))
        if error.IsError():
            raise SpssError(error)
    ...
    def SetVarLabel(self,varName,varLabel):
        ...
        error.SetErrorCode(PyInvokeSpss.SetVarLabel(varName,varLabel))
        if error.IsError():
            raise SpssError(error)
    ...
    def SetVarCValueLabel(self,varName,value,label):
        ...
        error.SetErrorCode(PyInvokeSpss.SetVarCValueLabel(varName,value,label))
        if error.IsError():
            raise SpssError(error)
    ...
    def SetVarNValueLabel(self,varName,value,label):
        ...
        error.SetErrorCode(PyInvokeSpss.SetVarNValueLabel(varName,nValue,label))
        if error.IsError():
            raise SpssError(error)
    ...
    def SetVarCMissingValues(self,varName,missingVal1=None,missingVal2=None,missingVal3=None):
        ...
        if format == 0:
            error.SetErrorCode(PyInvokeSpss.SetVarCMissingValues(varName,format))
        elif format == 1:
            error.SetErrorCode(PyInvokeSpss.SetVarCMissingValues(varName,format,temp[0]))
        elif format == 2:
            error.SetErrorCode(PyInvokeSpss.SetVarCMissingValues(varName,format,temp[0],temp[1]))
        elif format == 3:
            error.SetErrorCode(PyInvokeSpss.SetVarCMissingValues(varName,format,temp[0],temp[1],temp[2]))
    ...
    def SetVarNMissingValues(self,varName,missingFormat,missingVal1=None,missingVal2=None,missingVal3=None):
        ...
        if not isinstance(missingFormat,int):
            error.SetErrorCode(1000)
            raise SpssError(error)
        ...
        if missingVal1 and not isinstance(missingVal1,(float,int)):
            error.SetErrorCode(1033)
            raise SpssError(error)

        if missingVal2 and not isinstance(missingVal2,(float,int)):
            error.SetErrorCode(1033)
            raise SpssError(error)

        if missingVal3 and not isinstance(missingVal3,(float,int)):
            error.SetErrorCode(1033)
            raise SpssError(error)
        ...
        if abs(format) == 0:
            error.SetErrorCode(PyInvokeSpss.SetVarNMissingValues(varName,format))
        elif abs(format) == 1:
            error.SetErrorCode(PyInvokeSpss.SetVarNMissingValues(varName,format,temp[0]))
        elif abs(format) == 2:
            error.SetErrorCode(PyInvokeSpss.SetVarNMissingValues(varName,format,temp[0],temp[1]))
        elif abs(format) == 3:
            error.SetErrorCode(PyInvokeSpss.SetVarNMissingValues(varName,format,temp[0],temp[1],temp[2]))
    ...
    def SetVarMeasureLevel(self,varName,measureLevel):
        ...
        if not isinstance(measureLevel,int):
            error.SetErrorCode(1000)
            raise SpssError(error)

        error.SetErrorCode(PyInvokeSpss.SetVarMeasureLevel(varName,measureLevel))
        if error.IsError():
            raise SpssError(error)
    ...
    def SetVarAlignment(self,varName,aligment):
        ...
        if not isinstance(aligment,int):
            error.SetErrorCode(1000)
            raise SpssError(error)

        error.SetErrorCode(PyInvokeSpss.SetVarAlignment(varName,aligment))
        if error.IsError():
            raise SpssError(error)
    ...
    def SetVarFormat(self,varName,type,width,decimal=None):
        ...
        error.SetErrorCode(PyInvokeSpss.SetVarFormat(varName,type,width,decimal))
        if error.IsError():
            raise SpssError(error)
    ...
    def SetVarAttributes(self,varName,attrName,attrValue,index):
        ...
        error.SetErrorCode(PyInvokeSpss.SetVarAttributes(varName,attrName,attrValue,index))
        if error.IsError():
            raise SpssError(error)
    ...
    def CommitDictionary(self):
        ...
        error.SetErrorCode(PyInvokeSpss.CommitHeader())
        if error.IsError():
            raise SpssError(error)
    ...
    def CommitManyCases(self,newVars,caseData):
        ...
        if self.isBinary:
            ...
            error.SetErrorCode(PyInvokeSpss.CommitManyCases(tuple(newVarList),caseData,tuple(newVarListType)))
            if error.IsError():
                raise SpssError(error)                 
    ...
    def CommitCase(self):
        ...
        if self.isBinary:
            ...
            error.SetErrorCode(PyInvokeSpss.CommitCaseRecord())
            if error.IsError():
                raise SpssError(error)
    ...
    def SetVarRole(self,varName,varRole):
        ...
        error.SetErrorCode(PyInvokeSpss.SetVarRole(varName,varRoleNo))
        if error.IsError():
            raise SpssError(error)
...
class AppendCursor(BaseCursor,BaseWriteAppendCursor):
    ...
    def EndChanges(self):
        ...
        if self.isBinary:
            ...
            if not self.binaryStream.getNewCaseCount() > 0:
                error.SetErrorCode(75)
                if error.IsError():
                    raise SpssError(error)
            error.SetErrorCode(PyInvokeSpss.EndChanges())
            if error.IsError():
                raise SpssError(error)
        else:
            error.SetErrorCode(PyInvokeSpss.EndChanges())
    ...
    def CommitCase(self):
        ...
        if self.isBinary:
            ...
            error.SetErrorCode(PyInvokeSpss.CommitNewCase())
            if error.IsError():
                raise SpssError(error)
...
class Cursor(object):
    ...
    def __new__(self, var = None, accessType = "r", cvtDates=(), isBinary = True):
        ...
        if len(spss.spss.ProcDictionary) > 0:
            isBinary = False
