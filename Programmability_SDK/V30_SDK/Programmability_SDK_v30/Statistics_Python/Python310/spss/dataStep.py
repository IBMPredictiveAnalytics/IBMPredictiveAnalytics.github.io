...
class Dataset(object):
        ...
        if self.__isEmpty == False:
            ...
            if name == "*":
                ...
                if temp[1] == 0:
                    handle = temp[0]
            if handle != "*":
                for dds in list(PythonDatasets.values()):
                    if dds.name == handle:
    ...
    class FileAttribute:
        ...
        def clear(self):
            ...
            for attname in list(self.data.keys()):
                errLevel = PyInvokeSpss.DelDataFileAttributesInDS(spssutil.CheckStr(self.__ds.name), attname)
    ...
    class MResponseSet:
        ...
        def __getitem__(self, mrsetName):
            ...
            if not isinstance(mrsetName, str):
...
##                    raise SpssError, error
        ...
        def clear(self):
            ...
            for mrsetName in list(self.data.keys()):
                errLevel = PyInvokeSpss.DelMultiResponseSetInDS(spssutil.CheckStr(self.__ds.name), mrsetName)
...
class VariableList(object):
    ...
    def __init__(self, ds):
        ...
        if not self.__ds._Dataset__isEmpty:
            for varindex in range(varcount):
...
class CaseList(object):
    ...
    def __getitem__(self, indexOrCoordinate):
        ...
        def __getbyindex(indexOrCoordinate, part=None):
            if isinstance(indexOrCoordinate, int):
        ...
        if out is not None:
            ...
            Columnslice = False
            if isinstance(indexOrCoordinate[1],int):
    ...
    def __setitem__ (self, indexOrCoordinate, value):
        ...
        def __setbyindex(indexOrCoordinate, value, part=None):
            ...
            if isinstance(indexOrCoordinate, int):
                ...
                j=0
                for i in indices:
                    if isinstance(value[j], (int, float)):
                        value[j] = [value[j]]
    ...
    def __setrow(self, row, values, part=None):
        ...
        if len(values) > len(self.__vartype):
            ...
            indexLen = len(values)
        r = ( part==None and [range(indexLen)] or [part])[0]
...
class Variable(object):
            ...
            if vartype == 0:
                for value in values:
                    if not isinstance(value, (int,float)):
                        error.SetErrorCode(1033)
                        raise SpssError(error)
                labels = list(map(spssutil.CheckStr,labels))
                ...
                values = list(map(spssutil.CheckStr,values))
    ...
    def _setvalueLabels(self, valueLabels):
        ...
        num = len(list(valueLabels.items()))
        ...
        if num < smallSize:
            for x in list(valueLabels.items()):
                self.__vallbl.__setitem__(x[0], x[1])
        else:
            loop = num // groupSize
            for i in range(loop):
                values = list(valueLabels.keys())[i*groupSize:(i+1)*groupSize]
                labels = list(valueLabels.values())[i*groupSize:(i+1)*groupSize]
                self.__vallbl.__setitems__(values, labels)

            if num > loop * groupSize:
                values = list(valueLabels.keys())[loop*groupSize:]
                labels = list(valueLabels.values())[loop*groupSize:]
                self.__vallbl.__setitems__(values, labels)
