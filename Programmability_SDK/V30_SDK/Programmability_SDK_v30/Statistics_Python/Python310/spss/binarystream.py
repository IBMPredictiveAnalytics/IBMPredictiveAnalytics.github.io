...
class BaseBinary(object):
    ...
    def unpackdata(self, binarydata):
        ...
        strVarList = [i for i in range(len(self.varType)) if self.varType[i] > 0]
        for i in strVarList:
...
class CursorBinary(BaseBinary):
    def __init__(self, var = None, accessType = "r", pos = 0):
        ...
        for index in range(len(self.userMissing)):
    ...
    def unpackWriteData(self, binarydata):
        ...
        strVarList = [i for i in range(len(self.writeVarType)) if self.writeVarType[i] > 0]
        for i in strVarList:
    ...
    def packdataForWrite(self, v1):
        try:
            condition = (not self.cursorPos < 0) and (self.cursorPos < self.totalCaseCount + len(self.splitIndexes))
            if condition:
