...
def fixMacCert():
    ...
    try:
        ...
        # The certifi module should be installed for this to work
        return
    ...
    if len(preaction.wop) > 0:
        ...
        for item in text:
            ...
            for i in range(loop):
                ...
                error.SetErrorCode(PyInvokeSpss.PostSpssOutput(temp,len(temp.encode('utf-8'))))
            if rm > 0:
                temp=item[-rm:]
                temp = spssutil.CheckOutputStrU(temp)
                error.SetErrorCode(PyInvokeSpss.PostSpssOutput(temp,len(temp.encode('utf-8'))))
...
def StartSPSS(hide="",show="",notes=False, nfc=True, nl=True):
    ...
    if cur_locale == "" or cur_locale == "C" or cur_locale is None:
        ...
        PyInvokeSpss.Submit(cmd,len(cmd.encode('utf-8')))
...
def PostOutput(text):
    ...
    error.SetErrorCode(PyInvokeSpss.PostSpssOutput(text,length))
    if error.IsError():
        raise SpssError(error)
...
def Submit(cmdList):
    ...
    #Check the type of the argument
    if not isinstance(cmdList,(str,list,tuple)):
    ...
    for cmd in tempList[:-1]:
        error.SetErrorCode(PyInvokeSpss.QueueCommandPart(cmd,len(cmd.encode('utf-8'))))
        if error.IsError():
            raise SpssError
    ...
    error.SetErrorCode(PyInvokeSpss.Submit(tempList[-1],len(tempList[-1].encode('utf-8'))))

    if error.IsError():
        raise SpssError(error)

    if not PyInvokeSpss.IsUTF8mode():
        error.SetErrorCode(1075)
        raise SpssError(error)
...
def StartProcedure(procedureName, omsIdentifier=""):
    ...
    for dataset in list(PythonDatasets.values()):
        del dataset
...
def GetDatasets():
    ...
    out = list(PythonDatasets.values())
    return out
